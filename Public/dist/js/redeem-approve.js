...
var Apply = (function() {
    ...
    var conf = ['撤销审核？','结算？','不予结算？'];
    var audit_action = function(applyid,sid) {
        bootbox.confirm(conf[sid], function(result) {
            if (result) {
    ...
    return {
        init: function(initObj) {
            ...
            $(mainTable).on('click', '.link-not-audit', function(e) {
                audit_action($(this).attr('apply'),0);
            });
            $(mainTable).on('click', '.link-exchange', function(e) {
                audit_action($(this).attr('apply'),1);
            });
            $(mainTable).on('click', '.link-not-exchange', function(e) {
                audit_action($(this).attr('apply'),2);
            });
